...
**สถานะ: ทำครบทั้ง 5 ระบบแล้ว** — เอกสารนี้ตอนนี้เป็น reference ว่าแต่ละระบบต่อ
ยังไงและทำไม (ไฟล์/เหตุผล) มากกว่าจะเป็น to-do list ทดสอบ end-to-end ผ่านแล้วทั้ง
5 ระบบ (login จริง → เลือก severity → ส่ง → ยืนยันข้อมูลถูกต้องที่ฝั่ง
issue-service รวมไฟล์แนบ)
